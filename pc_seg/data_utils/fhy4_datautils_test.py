'''
data utils used in PointNet and its variants' training
...
map_kitti2mini = {
    'road':     'road',
     ...
     'traffic_sign': 'others',
     'vegetation':   'vegetation', # should change to terrain
     'terrain':      'terrain',# should change to terrain
     'sky':          'others',
     'person':    'others',
     'rider':     'others',
     'car':       'others',
...
class Semantic_KITTI_Utils():
    ...
    def get_pts_l(self, part, index, channels = 4, load_image = False, old = True):
        if load_image:
            ...
            # self.frame = cv2.cvtColor(self.frame, cv2.COLOR_BGR2RGB)
            # self.frame_HSV = cv2.cvtColor(self.frame_BGR, cv2.COLOR_BGR2HSV)
        fn_velo = os.path.join(self.data_path, 'label' if old else "pts_label", str(part))
        data = np.load(fn_velo+'/pts_l%06d.npz' % (index))
        raw_pts = data['pts_l']

        points = raw_pts[:,:channels]
        labels = raw_pts[:,-2].astype(np.int32)

        # if channels == 4:
        #     # points_label = raw_pts[:,:-1]
        #     points = raw_pts[:,channels]
        #     labels = raw_pts[:,-2].astype(np.int32)
        # elif channels == 3:
        #     points = raw_pts[:,channels]
