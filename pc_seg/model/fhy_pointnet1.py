...
class STN3d(nn.Module):
    def __init__(self):
        ...
        self.conv3 = torch.nn.Conv1d(128, 1024, 1)#512
        ...
        self.bn3 = nn.BatchNorm1d(1024)#512
...
class STNkd(nn.Module):
    def __init__(self, k=64):
        ...
        self.fc1 = nn.Linear(1024, 512)
        self.fc2 = nn.Linear(512, 256)
        self.fc3 = nn.Linear(256, k * k)#256
        self.relu = nn.ReLU()
...
class PointNetEncoder(nn.Module):
    def __init__(self, global_feat=True, input_dims = 4, feature_transform=False, light=False):
        super(PointNetEncoder, self).__init__()
        self.global_feat_size = 1024 if not light else 512
        self.stn = STNkd(k = input_dims)
        self.conv1 = torch.nn.Conv1d(input_dims, 64, 1)
        self.conv2 = torch.nn.Conv1d(64, 128, 1)
        self.conv3 = torch.nn.Conv1d(128, self.global_feat_size, 1)#512 
        self.bn1 = nn.BatchNorm1d(64)
        self.bn2 = nn.BatchNorm1d(128)
        self.bn3 = nn.BatchNorm1d(self.global_feat_size)#512
    ...
    def forward(self, x):
        ...
        x = torch.max(x, 2, keepdim=True)[0]
        x = x.view(-1, self.global_feat_size)#512
        #print('after max', x.shape)
        if self.global_feat:
            return x, trans, trans_feat
        else:
            x = x.view(-1, self.global_feat_size, 1).repeat(1, 1, n_pts)#512
...
class PointNetSeg(nn.Module):
    def __init__(self,num_class, input_dims=4, feature_transform=False):
        ...
        '''conv0'''
        self.conv0 = torch.nn.Conv1d(1088, 512, 1)
        self.conv1 = torch.nn.Conv1d(512, 256, 1)#(576, 256, 1)
        self.conv2 = torch.nn.Conv1d(256, 128, 1)#256
...
class PointNetSegLight(nn.Module):
    def __init__(self,num_class, input_dims=4, feature_transform=False):
        super(PointNetSegLight, self).__init__()
        self.k = num_class
        self.feat = PointNetEncoder(global_feat=False,input_dims = input_dims, feature_transform=feature_transform, light=True)
        '''conv0'''
        # self.conv0 = torch.nn.Conv1d(1088, 512, 1)
        self.conv1 = torch.nn.Conv1d(576, 256, 1)#(576, 256, 1)
        self.conv2 = torch.nn.Conv1d(256, 128, 1)#256
        self.conv3 = torch.nn.Conv1d(128, self.k, 1)
        '''bn0'''
        # self.bn0 = nn.BatchNorm1d(512)
